...
if not st.session_state.logged_in:
    ...
    # 📊 لوحة الإحصائيات العامة والمالية للموقع (تحديث فوري وتلقائي للـ 500 ريال عند اختيار نعم)
    ...
    # ------------------------------------------
    # 2. إدارة وتعديل وتحرير بيانات الأعضاء (مؤمنة بنظام النماذج الفوري المستقر 100%)
    # ------------------------------------------
    with tab2:
        if st.session_state.editing_member_idx is not None:
            ...
            # عزل لوحة التحرير بداخل استمارة مستقلة لضمان عمل زر الحفظ ومنع التعليق
            with st.form("form_edit_member_secure"):
                ...
                edit_paid = st.selectbox("تعديل حالة دفع الصندوق (+500 ريال تلقائياً عند نعم):", ["نعم", "لا"], index=["نعم", "لا"].index(current_member_data["تم دفع الصندوق"]))
