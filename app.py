تفضل الكود المحدث والكامل لملف app.py. تم تنظيم الموقع وترتيبه بالكامل باستخدام تبويبات واضحة وأعمدة متناسقة، مع إضافة الحقول الجديدة المطلوبة (الاسم، كود العائلة، حالة دفع الصندوق، والجنس)، بالإضافة إلى نظام فرز وتصفية متقدم للتقارير [1.1.1، 1.1.3].
## الكود البرمجي الجديد والمنظم لـ app.py
...
st.markdown("---")
# 2. إدارة قاعدة البيانات المؤقتة (Session State)# نستخدم هيكلة تحتوي على (الاسم، كود العائلة، حالة الدفع، الجنس)if 'members_db' not in st.session_state:
...
# تحويل البيانات الحالية إلى DataFrame لسهولة المعالجة والفرزdf_current = pd.DataFrame(st.session_state.members_db)
# إنشاء التبويبات لتنظيم واجهة الموقع بشكل مرتب ومنفصلtab_calc, tab_manage, tab_reports = st.tabs([
    "💰 1. حساب وتقسيم المبالغ", 
    "👥 2. إدارة وإضافة الأعضاء", 
    "📊 3. التقارير والفرز المتقدم"
])
# ==========================================# التبويب الأول: الحسابات والتقسيم المالي# ==========================================with tab_calc:
...
# ==========================================# التبويب الثاني: إدارة وإضافة الأعضاء مع الحذف# ==========================================with tab_manage:
...
# ==========================================# التبويب الثالث: التقارير والفرز المتقدم# ==========================================with tab_reports:
...
## التغييرات المطبقة لجعل الموقع مرتباً وبالمواصفات المطلوبة:

   1. واجهة شديدة الترتيب والتنظيم: تقسيم النظام بالكامل إلى 3 تبويبات واضحة (تبويب الحسابات، تبويب الإدارة الفورية، وتبويب التقارير والفرز).
   2. استمارات إدخال غنية بالبيانات: عند إضافة أي عضو جديد، يتيح لك النظام إدخال: الاسم، وكود العائلة، وحالة دفع الصندوق (نعم/لا)، والجنس (ذكر/أنثى).
   3. محرك تقارير وفرز متطور ذكي:
   * فرز أبجدي: ترتيب الأسماء من الألف إلى الياء تلقائياً بنقرة واحدة.
      * فرز حسب كود العائلة: تجميع أفراد كل عائلة معاً بشكل متتالي.
      * تصفية دقيقة وفلاتر: إمكانية عرض عائلة محددة فقط، أو استعراض من دفعوا الصندوق فقط، أو الفرز بناء على الجنس.
   4. تحميل التقارير المفرزة: يمكن تحميل الكشف النهائي المصفى فقط ومباشرة بصيغة متوافقة تماماً مع برنامج Excel وباللغة العربية.

الخطوة التالية: اذهب إلى حسابك في GitHub، وافتح ملف app.py، واستبدل محتواه بالكامل بهذا الكود المرتب لتنتهي المشكلة فوراً ويتحدث موقعك!
