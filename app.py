...
if not st.session_state.logged_in:
    ...
    # إنشاء التبويبات القياسية المعزولة كلياً
    ...
    # ------------------------------------------
    # 1. تبويب التقسيم المالي وإصدار مستند PDF المخصص للمناسبة
    # ------------------------------------------
    with tab1:
    ...
    with tab2:
        ...
        st.caption("💡 **طريقة العمل المباشرة:** اضغط على **(➕ Add row)** بالأسفل للإضافة، أو اضغط مرتين على أي خلية للتحرير والتعديل، أو حدد السطر واضغط Delete للحذف.")
        ...
        if not edited_members_df.equals(df_members_editor):
            ...
            st.success("تم حفظ البيانات وتحديث أموال الصندوق تلقائياً.")
            st.rerun()

    # ------------------------------------------
    # 3. تبويب لوحة التقارير والفرز الشامل وربط خيارات الفرز بملفات التنزيل
    # ------------------------------------------
    with tab3:
        ...
        with col_f1: 
            sort_opt = st.selectbox("🎯 نظام ترتيب وفرز الكشف الحالي والملفات:", ["بدون ترتيب", "أبجدي (من أ إلى ي)", "حسب كود العائلة", "عشوائي (خلط عشوائي)"], key="sort_sel_tab3")
        with col_f2: 
        ...
        with col_f3: 
            ...
            family_filter = st.selectbox("🏠 تصفية مخصصة لعائلة محددة:", unique_families, key="family_sel_tab3")
            
        # بناء مصفوفة التقارير المستقلة وقراءة التحديث الفوري للفلاتر
        df_rep = pd.DataFrame(st.session_state.members_list)
        
        if not df_rep.empty:
            # تطبيق تصفية الجنس وتصفية كود العائلة
            if gender_filter != "الكل": df_rep = df_rep[df_rep["الجنس"] == gender_filter]
